...
// Funções para converter PDF em Base64 e persistir entre sessões
...
async function lessonFileChosen(lid,input){
  const file=input.files[0]; if(!file) return;
  try {
    const base64 = await fileToBase64(file);
    input._base64Data = base64;
...
function saveCourse(){
  ...
  mEls.forEach((mel,mi)=>{
    ...
    mel.querySelectorAll('.lesson-item').forEach((lel,li)=>{
      ...
      const url=fileInput?._base64Data||lurl;
